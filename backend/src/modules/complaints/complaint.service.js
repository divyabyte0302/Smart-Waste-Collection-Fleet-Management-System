...
class ComplaintService {
  ...
  static async createComplaint(user, data) {
    ...
    const created = await ComplaintModel.create(complaintData);
    snsService.notifyComplaintStatusUpdate(created, 'None', user.name)
      .catch(err => console.warn('[SNS] Notification notice:', err.message));
    return created;
  }
  ...
  static async updateComplaint(id, updates, currentUser) {
    ...
    const updated = await ComplaintModel.update(existing.id, sanitizedUpdates);
    if (updates.assignedStaffId) {
      snsService.notifyComplaintStatusUpdate(updated, existing.status, currentUser.name)
        .catch(err => console.warn('[SNS] Notification notice:', err.message));
    }
    return updated;
  }
  ...
  static async updateComplaintStatus(id, newStatus, note, currentUser) {
    ...
    const updated = await ComplaintModel.update(existing.id, { status: newStatus });
    snsService.notifyComplaintStatusUpdate(updated, existing.status, currentUser.name)
      .catch(err => console.warn('[SNS] Notification notice:', err.message));
    return updated;
  }
