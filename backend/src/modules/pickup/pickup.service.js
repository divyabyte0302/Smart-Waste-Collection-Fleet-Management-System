...
class PickupService {
  ...
  static async getPickupById(id, currentUser) {
    ...
    if (currentUser && currentUser.role === ROLES.CITIZEN && pickup.citizenId !== currentUser.id) {
  ...
  static async updatePickup(id, updates, currentUser) {
    ...
    const updated = await PickupModel.update(existing.id, sanitized);

    if (sanitized.assignedStaff && sanitized.assignedVehicle) {
      snsService.notifyPickupAssignment(updated, sanitized.assignedVehicle, sanitized.assignedStaff)
        .catch(err => console.warn('[SNS] Pickup notice:', err.message));
    }
  ...
  static async updateStatus(id, newStatus, currentUser) {
    ...
    const updated = await PickupModel.update(existing.id, { status: newStatus });

    // SNS Event Triggers
    if (newStatus === PICKUP_STATUS.APPROVED) {
      snsService.notifyPickupApproval(updated, currentUser.name)
        .catch(err => console.warn('[SNS] Pickup notice:', err.message));
    } else if (newStatus === PICKUP_STATUS.COMPLETED) {
      snsService.notifyPickupCompletion(updated, currentUser.name)
        .catch(err => console.warn('[SNS] Pickup notice:', err.message));
    } else if (newStatus === PICKUP_STATUS.ASSIGNED && updated.assignedVehicle && updated.assignedStaff) {
      snsService.notifyPickupAssignment(updated, updated.assignedVehicle, updated.assignedStaff)
        .catch(err => console.warn('[SNS] Pickup notice:', err.message));
    }
